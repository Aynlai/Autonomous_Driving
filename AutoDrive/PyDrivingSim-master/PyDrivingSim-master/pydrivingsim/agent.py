...
class Agent:
    ...
    def __compute(self, v: Vehicle):
        ...
        ## TODO
        # trafficlight_list = []
        # trafficlight_dist_list = []
        for obj in World().obj_list:
            if type(obj) is TrafficLight:
                # Get the closest trafficlight in front
                if trafficlightDist == 0 or (
                    obj.pos[0] - v.state[0] > -1.0
                    and obj.pos[0] - v.state[0] < trafficlightDist
                ):
            ...
            #         if delta_x > -1.0:  # Only consider traffic lights ahead
            #             trafficlight_list.append(obj)
            #             trafficlight_dist_list.append(delta_x)
        ...
        s.NrTrfLights = 0
        # if trafficlight_list:
        #     closest_idx = trafficlight_dist_list.index(min(trafficlight_dist_list))
        #     closest_trafficlight = trafficlight_list[closest_idx]

        #     s.NrTrfLights = len(trafficlight_list)  # Total traffic lights
        #     s.TrfLightDist = trafficlight_dist_list[closest_idx]
        #     s.TrfLightCurrState = (
        #         closest_trafficlight.state + 1
        #     )  # Adjusted for simulation state
        #     s.TrfLightFirstTimeToChange = (
        #         closest_trafficlight.time_phases[closest_trafficlight.state]
        #         - closest_trafficlight.time_past_switch
        #     )
        #     s.TrfLightFirstNextState = divmod(closest_trafficlight.state + 1, 3)[1] + 1
        #     s.TrfLightSecondTimeToChange = (
        #         s.TrfLightFirstTimeToChange
        #         + closest_trafficlight.time_phases[
        #             divmod(closest_trafficlight.state + 1, 3)[1]
        #         ]
        #     )
        #     s.TrfLightSecondNextState = divmod(closest_trafficlight.state + 2, 3)[1] + 1
        #     s.TrfLightThirdTimeToChange = (
        #         s.TrfLightSecondTimeToChange
        #         + closest_trafficlight.time_phases[
        #             divmod(closest_trafficlight.state + 2, 3)[1]
        #         ]
        #     )
        ...
        if trafficlight:
            ...
            )  # 1 = Green, 2 = Yellow, 3 = Red, 0 = Flashing
            s.TrfLightFirstTimeToChange = (
                trafficlight.time_phases[trafficlight.state]
                - trafficlight.time_past_switch
            )
            s.TrfLightFirstNextState = divmod(trafficlight.state + 1, 3)[1] + 1
            s.TrfLightSecondTimeToChange = (
                s.TrfLightFirstTimeToChange
                + trafficlight.time_phases[divmod(trafficlight.state + 1, 3)[1]]
            )
            s.TrfLightSecondNextState = divmod(trafficlight.state + 2, 3)[1] + 1
            s.TrfLightThirdTimeToChange = (
                s.TrfLightSecondTimeToChange
                + trafficlight.time_phases[divmod(trafficlight.state + 2, 3)[1]]
            )
