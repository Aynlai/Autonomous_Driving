...
class World(metaclass=Singleton):
    ...
    def __init__(self):
        ...
        self.screen = pygame.display.set_mode(
            (self.__metadata["screen_size"][0], self.__metadata["screen_size"][1])
        )
        self.backgorund = pygame.image.load("imgs/bg.jpeg")

        # Video information
        self.scaling_factor = (
            self.__metadata["screen_size"][0] / self.__metadata["world_in_screen"]
        )
    ...
    def _render(self):
        ...
        screen_x_pos = (
            self.bg_pos[0]
            - self.world_pos[0] * self.scaling_factor
            + self.__metadata["screen_size"][0] / 2
        )
        # the minus sign there is because the graph y axis points downward.
        screen_y_pos = (
            self.bg_pos[1]
            + self.world_pos[1] * self.scaling_factor
            + self.__metadata["screen_size"][1] / 2
        )
    ...
    def update(self):
        ...
        if (not self.__metadata["final_time"] == "inf") and self.time > self.__metadata[
            "final_time"
        ]:
            self.loop = 0
